...
export interface OpenTicketsQueueState {
  sort: string;
  order: string;
  page: number;
  type: string;
...
function stateKey(s: OpenTicketsQueueState): string {
  return [s.sort, s.order, s.page, s.type, s.ownerId, s.number, s.q, s.privated].join(
    "|"
  );
}

export function useOpenTicketsQueue() {
  ...
  const fetchList = useCallback(
    async (options?: { silent?: boolean }) => {
      ...
      try {
        ...
        console.error("[open-tickets] fetch failed:", err);
      } finally {
